    ...
    if(this != NULL && this2 != NULL){
        ...
        for(int i=0; i < tam ; i++)
        {
            ...
            if( ll_contains(this, elemento) == 0)
            {
                returnAux = 0;
                break;
            } //this es la lista donde quiero buscar
        }
    }
...
/** \brief Filtra una lista y agrega los elementos a una lista nueva
 *
 * \param Linkedlist* this, lista a filtrar
 * \param int (*pFunc) (void*), puntero a funcion
...
LinkedList* ll_map(LinkedList* lista, int (*pFunc)(void)){
	...
	if(lista!=NULL && *pFunc!=NULL){
		...
		for (int i = 0; i < tam; i++) {
			...
			if(tiempo>=50 && tiempo<=120){
				bici_setTiempo(pElement, tiempo);
